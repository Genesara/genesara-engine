 ...
 *
 * The lazy-NPC-spawn overload (`applyEffects(effects, lazyNpcSpawn, rng)`) lives
 * in the umbrella `:world` module since it imports `LazyNpcSpawnHook` from
 * `:world-environment`.
 */
fun WorldState.applyEffects(effects: List<CrossZoneEffect>): WorldState =
    effects.fold(this) { state, effect -> state.apply(effect) }

fun WorldState.apply(effect: CrossZoneEffect): WorldState = when (effect) {
