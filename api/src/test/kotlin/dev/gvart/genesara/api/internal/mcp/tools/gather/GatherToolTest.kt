...
class GatherToolTest {
    ...
    @Test
    fun `queues a GatherResource command at the next tick and returns the ack`() {
        val tool = GatherTool(gateway, tickClock, activity)

        val response = tool.invoke(GatherRequest(itemId = "WOOD"), toolContext)

        assertEquals("queued", response.kind)
        assertEquals("WOOD", response.itemId)
        assertEquals(51L, response.appliesAtTick)
        val (cmd, appliesAt) = gateway.submissions.single()
        val gather = assertNotNull(cmd as? WorldCommand.GatherResource)
        assertEquals(agent, gather.agent)
        assertEquals(ItemId("WOOD"), gather.item)
        assertEquals(51L, appliesAt)
        assertEquals(gather.commandId, response.commandId)
    }
    ...
    fun `touches activity registry on every successful invocation`() {
        ...
        // Before invoke: registry has no record → agent treated as stale relative to a future cutoff.
        assertTrue(agent !in activity.staleAgents(clock.instant().minusSeconds(60)))

        tool.invoke(GatherRequest(itemId = "WOOD"), toolContext)

        // Stale relative to a future cutoff (sanity: a touch was recorded).
